...
namespace Manipulation
{
	public static class VisualizerTask
	{
		...
		public static void KeyDown(Form form, KeyEventArgs key)
		{
			...
			if (flag) Wrist = -Alpha - Shoulder - Elbow;
            form.Invalidate(); 
		}
		...
		{
			double[] coor = ManipulatorTask.MoveManipulatorTo(X, Y, Alpha);
            if (coor[0] != double.NaN && coor[1] != double.NaN && coor[2] != double.NaN)
			{
				Shoulder = coor[0];
				Elbow = coor[1];
				Wrist = coor[2];
        ...
        public static void DrawManipulator(Graphics graphics, PointF shoulderPos)
		{
			...
			{
				points[i] = ConvertMathToWindow(joints[i-1], shoulderPos);
                graphics.FillEllipse(JointBrush, points[i].X - 10, points[i].Y - 10, 20, 20);
            }
